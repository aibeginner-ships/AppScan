# AppScan v3.1.1 - App Review Summarizer with Enhanced Semantic Clustering

## Overview

AppScan is a web application designed to analyze user reviews from Google Play Store and Apple App Store. It provides instant, AI-powered insights, including sentiment percentages, monthly trends, categorized feedback, rating analysis, and a list of top negative reviews. **Version 3.1.1** enhances semantic clustering with quality improvements for quote alignment, action specificity, and cluster purity. The project aims to give users clear understanding of what customers appreciate and dislike about their applications.
...
### Insight Generation System (v3.1.1)

**Version 3.1.1** introduces a semantic clustering pipeline with enhanced quality improvements:
...
1. **Semantic Clustering** (`semanticClusterer.ts`):
   ...
   - **NEW in v3.1.1**: Cluster refinement pass to split mixed-topic clusters using 8 keyword themes
...
2. **Insight Refinement** (`insightRefiner.ts`):
   ...
   - **NEW in v3.1.1**: Text similarity-based quote selection using Jaccard index
   - **NEW in v3.1.1**: Enhanced prompts with top keywords for better context
   - **NEW in v3.1.1**: Specific action generation with 10 concrete templates (crashes, performance, login, ads, pricing, features, UI, support, updates)
...
**Key Improvements (v3.1.1)**:
- **Quote alignment**: Representative quotes semantically match insight themes via text similarity
- **Action specificity**: Concrete, actionable suggestions (no generic "Prioritize fixing..." templates)
- **Cluster purity**: Keyword-based refinement prevents mixed-topic clusters
